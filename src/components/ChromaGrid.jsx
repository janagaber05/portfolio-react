  ...
  return (
    ...
    >
      {data.map((card, index) => {
        ...
        const spotlight = card.spotlight || 'rgba(255, 255, 255, 0.3)';
        return (
          <article
            key={`${card.title}-${index}`}
            className={`chroma-card${compact ? ' chroma-card--compact' : ''}`}
            onMouseMove={handleCardMove}
            onClick={() => handleCardClick(card.url)}
            style={{
              '--card-border': card.borderColor || 'transparent',
              '--card-gradient': card.gradient || 'linear-gradient(160deg, #1f1b2e, #05030f)',
              '--spotlight-color': spotlight,
              cursor: card.url ? 'pointer' : 'default',
            }}
          >
            {compact ? (
              <div className="chroma-compact">
                <span className="chroma-compact__title">{card.title}</span>
                {card.subtitle && (
                  <span className="chroma-compact__subtitle">{card.subtitle}</span>
                )}
              </div>
            ) : (
              <>
